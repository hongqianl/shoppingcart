...
export function Header() {
  ...
  return (
    ...
    <div className="flex md:order-2">
        ...
        <span className='p-2 text-lg font-bold dark:text-white'>Cart:  {numberOfItems}</span>
    ...
    <div className={`${hideMenu ? 'hidden' : ''} items-center justify-between w-full md:flex md:w-auto md:order-1`} id="navbar-cta">
        ...
        <li>
            <NavLink exact to="/" className={({isActive})=> isActive ? activeClass :inactiveClass} aria-current="page">Home</NavLink>
        </li>
        <li>
            <NavLink exact to="/cart" className={({isActive}) =>isActive ? activeClass :inactiveClass}>Cart</NavLink>
        </li>
        </ul>
    </div>
    </div>
